...
- (void)viewDidLoad
{
    ...
    // Binding background searchText with signal
    RAC(self.searchText, backgroundColor) = [self.searchText.rac_textSignal
                                             map:^id(NSString *text) {
                                                 return [self isValidSearchText:text] ? [UIColor whiteColor] : [UIColor yellowColor];
                                             }];
    
    // Initialize Account Store and Twitter Account Identifier
    ...
    [[[[[[[self requestAccessToTwitterSignal] then:^RACSignal *{
     ...
     ] subscribeNext:^(NSDictionary *twitterData) {
        @strongify(self);
        NSArray *statuses = twitterData[@"statuses"];
...
#pragma mark - Validate Search Text
- (BOOL)isValidSearchText:(NSString *)text {
    return text.length > 2;
}

#pragma mark - Twitter access signal
- (RACSignal *)requestAccessToTwitterSignal {
...
#pragma mark - Twitter search request
- (SLRequest *)requestForTwitterSearchWithText:(NSString *)text {
...
#pragma mark - Twitter search signal
- (RACSignal *)signalForSearchWithText:(NSString *)text {
